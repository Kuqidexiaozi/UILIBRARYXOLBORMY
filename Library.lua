...
local function GetSchemeValue(Index)
    ...
    if AliasIndex and Library.Scheme[AliasIndex] ~= nil then
        warn(string.format("配色方案值 %q 已弃用，请改用 %q。", Index, AliasIndex))
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
    warn("Obsidian:MakeOutline 已弃用，请改用 Obsidian:AddOutline")
...
function Library:AddDraggableLabel(...)
    ...
    assert(IconPosition == "left" or IconPosition == "right", "图标位置必须为 'left' 或 'right'")
    ...
    function DraggableLabel:SetIcon(NewIcon: string)
        ...
        if IsNotEmpty then
            ...
            assert(CustomIcon, "图标必须是有效的 Roblox 资源、有效的 URL 或有效的 Lucide 图标")
...
do
    ...
    function Library:SetWatermark(Text: string)
        warn("水印功能已弃用，请改用 Library:AddDraggableLabel")
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if ParentObj.Type == "Button" or ParentObj.Type == "SubButton" then
            assert(Info.Mode == "Press", "按钮上的按键绑定器只能使用 'Press' 模式")
        ...
        if KeyPicker.Mode == "Press" then
            assert(ParentObj.Type == "Label" or ParentObj.Type == "Button" or ParentObj.Type == "SubButton", "按键绑定器使用 'Press' 模式只能应用于标签和按钮")
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:SetupDependencies(Dependencies)
            for _, Dependency in Dependencies do
                assert(typeof(Dependency) == "table", "依赖项必须是表")
...
function Library:SetBackgroundImage(Image: string | number)
    assert(typeof(Image) == "string" or typeof(Image) == "number", "需要字符串或数字，实际得到 " .. typeof(Image))
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:ChangeTitle(title)
        assert(typeof(title) == "string", "标题需要字符串，实际得到: " .. typeof(title))
    ...
    function Window:SetFooter(Footer: string)
        assert(typeof(Footer) == "string", "页脚需要字符串，实际得到: " .. typeof(Footer))
    ...
    function Window:SetCornerRadius(Radius: number)
        assert(typeof(Radius) == "number", "圆角半径需要数字，实际得到: " .. typeof(Radius))
